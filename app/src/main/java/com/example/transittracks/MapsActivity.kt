...
interface StopDao{
    ...
    @Query("SELECT DISTINCT route.* FROM route INNER JOIN (SELECT * FROM trip WHERE tripID IN (SELECT DISTINCT tripID FROM StopTime WHERE stopID = :qstopID)) y ON route.routeID = y.routeID")
